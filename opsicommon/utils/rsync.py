...
def librsync_signature(filename: Path | str, base64_encoded: bool = True) -> str | bytes:
	...
	try:
		...
		raise RuntimeError(f"Failed to get librsync signature from {filename}: {forceString(sig_err)}") from sig_err
...
def librsync_delta_file(filename: Path | str, signature: bytes, deltafile: Path | str) -> None:
	...
	try:
		...
		raise RuntimeError(f"Failed to write delta file {deltafile}: {forceString(sig_err)}") from sig_err
...
def librsync_patch_file(oldfile: Path | str, deltafile: Path | str, newfile: Path | str) -> None:
	...
	try:
		...
		raise RuntimeError(f"Failed to patch file {oldfile}: {forceString(patch_err)}") from patch_err
